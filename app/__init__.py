...
def clients(): # Função de controlador das rotas do cliente
    ...
    if request.method == "POST":
        ...
        if args.get("username") != USERNAME or args.get("password") != PASSWORD: # Conferindo login com o env
...
def dowload(): # Função para fazer o dowload do arquivo exel
    args = request.args # Pegando o query
    if args.get("username") != USERNAME or args.get("password") != PASSWORD: # Verificando o login com o env
...
#     app.run('0.0.0.0',port=PORT,debug=True) # Rodando o servidor de